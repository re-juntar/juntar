            ...
            <div class="event-body bg-[#fff]">
                ...
                <div class="event-body-hero flex flex-col md:flex md:flex-row p-[15px] ">
                    <div class="general-info flex flex-col justify-around items-start w-full md:w-8/12 md:flex">
                        <p class="text-[16px] mb-[0.5rem] flex items-center justify-center"><i
                                class="fa-regular fa-calendar mr-3"></i>{{ $event->start_date }}</p>
                        <h2 class="text-[2rem] mb-[0.5rem] font-bold">{{ $event->name }}</h2>
                        <h3 class="text-[1rem]">Evento organizado por {{ $organizer->user_name }}
                            {{ $organizer->user_surname }}</h3>
                        @if ($coorganizers)
                    ...
                    <div
                        class="event-flyer-and-logo py-[1rem] flex flex-col items-center justify-center md:flex w-full md:w-4/12">
                        <div class="event-flyer">
                            @if ($event['image_flyer'])
                                <img class="rounded-lg w-full max-w-[80%] mx-auto" src="{{ asset($flyerSrc) }}"
                        ...
                        <div class="event-logo pt-[0.5rem]">
                            @if ($event['image_logo'])
                                <img class="rounded-lg max-w-[50%] mx-auto" src="{{ asset($logoSrc) }}"
                                    alt="Logo {{ $event->name }}">
                            @else
                                {{ $logoSrcNull }}
                            @endif
                ...
                @if (count($presentations))
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg px-[15px] ">
                        ...
                        <table class="w-full text-sm text-left text-fogra-darkish d text-[1rem]">
                            <thead
                                class="text-center text-xs bg-fogra-darkish text-white-ghost uppercase rounded-[1.25rem]">
                                <tr>
                            ...
                            <tbody>
                                @foreach ($presentations as $presentation)
                                    <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <th scope="row"
                                            class="text-left px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                            {{ $presentation->id }}</th>
                                        ...
                                        <td class="px-6 py-4"><a href="{{ $presentation->resources_link }}"><i
                                                    class="fa-solid fa-paperclip text-sky-600"></i></a></td>
