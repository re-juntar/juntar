...
<div>
    ...
    <x-hero>
        <h5
            class="text-white-ghost uppercase font-medium text-[1.5rem] md:text-[2.5rem] mb-4 text-center leading-[1.2]">
            {{ $event->name }}
        </h5>
        <h5 class="text-white-ghost uppercase font-medium text-[16px] md:text-[24px] mb-[0.5rem]"><i
                class="fa-regular fa-calendar mr-3"></i>{{ $event->start_date }}</h5>
        <h5 class="text-white-ghost uppercase font-medium text-[16px] md:text-[24px] mb-[0.5rem]"><i
                class="fa-solid fa-location-dot mr-3"></i>{{ $event->modality_description }}</h5>
    ...
    <div class="event bg-[#0B0D19] break-words">
        <div
            class="event-container max-w-md md:max-w-3xl lg:max-w-4xl xl:max-w-7xl mx-auto rounded-[0.25rem] py-[7vh] px-[3vh]">
            <div class="event-body bg-[#fff]">
                ...
                @if ($hasPermission)
                    <x-pink-header header class="h-[50px]" style="justify-content: start">
                        <a href="{{ route('edit-event', $event->id) }}" class="flex items-center">
                            <x-button
                                class="h-full hover:bg-fogra-darkish text-[12px] md:text-[1.25rem] text-sm py-[0rem] px-[0.3rem] sm:py-[0.1rem] sm:px-[0.5rem] md:py-[0.2rem] md:px-[0.5rem] lg:py-[0.1rem] lg:px-[0.5rem] xl:py-[0.5rem] xl:px-[1rem]">
                        ...
                        @if ($event->pre_registration)
                            <a href="{{ route('formbuilder', $event->id) }}" class="flex items-center">
                                <x-button
                                    class="h-full hover:bg-fogra-darkish text-[9px] sm:text-[1rem] md:text-[1.25rem]  text-sm py-[0rem] px-[0.1rem] sm:py-[0.1rem] sm:px-[0.1rem] md:py-[0.2rem] md:px-[0.5rem] lg:py-[0.1rem] lg:px-[0.5rem] xl:py-[0.5rem] xl:px-[1rem]">
                ...
                <div class="event-body-hero flex flex-col md:flex md:flex-row p-[15px] ">
                    <div class="general-info flex flex-col items-center lg:items-start w-full md:w-8/12 md:flex">
                        <p class="my-5 text-[16px] mb-[0.5rem] flex items-center justify-center"><i
                                class="fa-regular fa-calendar mr-3"></i>{{ $event->start_date }}</p>
                    ...
                    <div
                        class="event-flyer-and-logo flex flex-col items-center justify-center md:flex w-full md:w-4/12">
                        <div class="event-logo">
                            @if ($event['image_logo'])
                                <img class="rounded-lg max-w-[80%] mx-auto" src="{{ asset($logoSrc) }}"
                                    alt="Logo {{ $event->name }}">
                            @else
                                {{ $logoSrcNull }}
                            @endif
                ...
                <div
                    class="event-body-inscription px-[15px] flex flex-col justify-between sm:flex sm:flex-row py-[3vh] bg-[#F2F2F2]">
                    <div class="flex flex-col sm:flex sm:flex-row items-center justify-center md:flex">
                        ...
                        @if ($event->pre_registration && $event->inscription_end_date)
                            <span class="text-[#ff0000] font-bold">&nbsp;*Requiere pre-inscripción* *Fecha Limite:
                                {{ $event->inscription_end_date }}*</span></p>
                        @endif
                    </div>
                        ...
                        @if ($today <= $inscription_end_date && $event->capacity != 0 && $event->event_status_id == 1)
                            @if ($event->pre_registration)
                                @if (count($arrIsEnrolled) == 1)
                                    <x-button wire:click='confirmUnsuscribe({{ $event }})' class="text-[16px] ">
                                        Desinscribirse</x-button>
                                @else
                            ...
                            @else
                                @if (count($arrIsEnrolled) == 1)
                                    <x-button wire:click='confirmUnsuscribe({{ $event }})' class="text-[16px] ">
                                        Desinscribirse</x-button>
                                @else
                                    <x-button wire:click='confirmInscription({{ $event }})'
                ...
                <div class="pb-[3rem] p-[15px]">
                    <div class="flex items-center">
                        ...
                        @if ($hasPermission)
                            <a class="text-[2rem] ml-2" href="{{ route('create-presentation', $event->id) }}"><i
                                    class="fa-solid fa-plus fa-2xs"></i></a>
                        @endif
                    </div>
