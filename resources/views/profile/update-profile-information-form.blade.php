<x-jet-form-section submit="updateProfileInformation">
    ...
    <x-slot name="description">
        {{ __('Actualice la información y correo electrónico de su cuenta.') }}
    </x-slot>
    ...
    <x-slot name="form">
        ...
        @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
            <div x-data="{ photoName: null, photoPreview: null }" class="col-span-6 sm:col-span-4">
                ...
                <x-jet-label for="photo" value="{{ __('Imágen de Perfil') }}" />
                ...
                <x-jet-secondary-button class="mt-2 mr-2" type="button" x-on:click.prevent="$refs.photo.click()">
                    {{ __('Seleccione una nueva Imágen de Perfil') }}
                </x-jet-secondary-button>

                @if ($this->user->profile_photo_path)
                    <x-jet-secondary-button type="button" class="mt-2" wire:click="deleteProfilePhoto">
                        {{ __('Eliminar Imágen de Perfil') }}
                    </x-jet-secondary-button>
                @endif
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && !$this->user->hasVerifiedEmail())
                <p class="text-sm mt-2">
                    {{ __('Su correo electrónico aún no ha sido confirmado.') }}

                    <button type="button" class="underline text-sm text-gray-600 hover:text-gray-900"
                        wire:click.prevent="sendEmailVerification">
                        {{ __('Click aquí para re-enviar el correo electrónico de verificación.') }}
                    </button>
                </p>
